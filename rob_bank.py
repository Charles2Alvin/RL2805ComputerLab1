...
class Maze:
    ...
    def __move(self, state, action):
        ...
        # Reinitialise when caught by the police
        if i == m and j == n:
    ...
    def __move_police(self, state):
        ...
        # On the same row
        if i == m and j < n:
            action = np.random.choice([self.MOVE_UP, self.MOVE_DOWN, self.MOVE_LEFT])
        elif i == m and j > n:
            action = np.random.choice([self.MOVE_UP, self.MOVE_DOWN, self.MOVE_RIGHT])
        elif j == n and i < m:
            action = np.random.choice([self.MOVE_LEFT, self.MOVE_UP, self.MOVE_RIGHT])
        elif j == n and i > m:
            action = np.random.choice([self.MOVE_LEFT, self.MOVE_DOWN, self.MOVE_RIGHT])
        # right bottom
        elif i < m and j < n:
            action = np.random.choice([self.MOVE_UP, self.MOVE_LEFT])
        # left bottom
        elif i < m and j > n:
            action = np.random.choice([self.MOVE_UP, self.MOVE_RIGHT])
        # right upper
        elif i > m and j < n:
            action = np.random.choice([self.MOVE_DOWN, self.MOVE_LEFT])
        # left upper
        else:
            action = np.random.choice([self.MOVE_DOWN, self.MOVE_RIGHT])
    ...
    def __rewards(self):
        ...
        for s in range(self.n_states):
            for a in range(self.n_actions):
                ...
                one_step_from_police = (np.abs(i - m) + np.abs(j - n)) == 1
    ...
    def draw_optimal_policy(self, maze, police, policy):
        m = police[0]
        n = police[1]

        # Map a color to each cell in the maze
        col_map = {0: WHITE, 1: LIGHT_GREEN, 2: LIGHT_RED}
        ...
        for i in range(rows):
            for j in range(cols):
                if maze[i, j] == 0:
                    state = (i, j, m, n)
                    policy_at_state = policy[self.map[state]]
                    text = col_map[policy_at_state]
                    grid.get_celld()[(i, j)].get_text().set_text(text)

        grid.get_celld()[(0, 0)].get_text().set_text("Bank")
        grid.get_celld()[(0, 5)].get_text().set_text("Bank")
        grid.get_celld()[(2, 0)].get_text().set_text("Bank")
        grid.get_celld()[(2, 5)].get_text().set_text("Bank")

        grid.get_celld()[(1, 2)].get_text().set_text("PS")
        grid.get_celld()[(m, n)].get_text().set_text("Police")
...
def animate_solution(maze, path):
    # Map a color to each cell in the maze
    col_map = {0: WHITE, 1: LIGHT_GREEN, 2: LIGHT_RED, 3: LIGHT_PURPLE, 5: LIGHT_ORANGE}
    ...
    for i in range(len(path)):
        ...
        police_now = path[i][2:4]
        police_before = path[i - 2][2:4]

        if i == 0:
            grid.get_celld()[police_now].set_facecolor(RED)
            grid.get_celld()[police_now].get_text().set_text('police')
            continue

        # Player's turn
        if not i % 2:
            if player_now == police_now:
                ...
                grid.get_celld()[player_now].get_text().set_text('CAUGHT')
                time.sleep(0.3)
                # grid.get_celld()[0, 0].get_text().set_text('Player')
                # grid.get_celld()[0, 0].set_facecolor(LIGHT_ORANGE)
                # grid.get_celld()[1, 2].get_text().set_text('Police')
            ...
            if player_now == police_now:
                ...
                grid.get_celld()[police_now].set_facecolor(RED)
                grid.get_celld()[police_now].get_text().set_text('Police')
            if not police_now == police_before:
                # Reset the color of last position
                grid.get_celld()[police_before].set_facecolor(col_map[maze[police_before]])
                grid.get_celld()[police_before].get_text().set_text('')
