...
class Maze:
    ...
    def __states(self):
        ...
        map = dict()
        s = 0
        n1, n2 = self.maze.shape[0], self.maze.shape[1]
        for i in range(n1):
            for j in range(n2):
                if self.maze[i, j] != 1:
                    for m in range(n1):
    ...
    def __move(self, state, action):
        """ Makes a step in the maze, given a current position and an action.
            ...
            :return tuple next_cell: Position (i,j,m,n) on the maze that agent transitions to.
        """
        # Get the player's position (i,j) and the minotaur's position (m,n)
        ...
        if hitting_maze_walls or eaten or success:
            return state
        else:
        ...
        if weights is None:
            for s in range(self.n_states):
                for a in range(self.n_actions):
                    ...
                    # The action brings the player to the same place
                    ...
                    # Eaten at the next state
                    elif i == m and j == n:
                        rewards[s, a] = self.MINOTAUR_REWARD

                    # Reward for reaching the exit
                    elif s == next_s and self.maze[i, j] == 2:
                        rewards[s, a] = self.GOAL_REWARD

                    # Reward for being one step away from the minotaur
